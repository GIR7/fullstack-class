...
export class Migration20230501062804 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('alter table "users" drop column "is_match";');
  }

  async down(): Promise<void> {
    this.addSql('drop table if exists "match" cascade;');

    this.addSql('alter table "users" add column "is_match" boolean not null default false;');
  }
